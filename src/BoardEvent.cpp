...
namespace chess {
    ...
    void ::chess::BoardEvent::handleMouseButtonEvent(Board& board, const sf::Event::MouseButtonEvent &event, bool isPressed) {
        float x = event.x;
        float y = event.y;
    ...
    void ::chess::BoardEvent::handleMouseMoveEvent(Board& board, const sf::Event::MouseMoveEvent &event) {
        if (draggedPieceArrayIndex != -1 && draggedPieceArrayIndex < board.pieces.size()) {
            board.pieces[draggedPieceArrayIndex].setPosition(sf::Vector2f(event.x, event.y) +
            mouseOffset);
        }
    }


    void ::chess::BoardEvent::findClickedPiece(Board& board, float x, float y) {
    ...
    void ::chess::BoardEvent::findLegalMoves(Board& board, int pieceIndex) {
        auto& piece = board.pieces[pieceIndex];

        possibleMoves = PieceMovement::findPossibleMovesForPieceType(piece, board);
    }


    void ::chess::BoardEvent::releasePiece(Board& board, float x, float y) {
        if (draggedPieceArrayIndex != -1) {
            ...
            if (isPossibleMove(board, col, row)) {
    ...
    bool ::chess::BoardEvent::isPossibleMove(Board& board, int col, int row) {
    ...
    void ::chess::BoardEvent::handleLegalMove(Board& board, int col, int row) {
    ...
    void ::chess::BoardEvent::updatePieceLayout(Board& board, int col, int row, const Piece& piece) {
    ...
    void ::chess::BoardEvent::removeTargetPiece(Board& board, int col, int row) {
    ...
    void ::chess::BoardEvent::revertMove(Board& board) {
        int oldCol = static_cast<int>(originalPos.x);
        int oldRow = static_cast<int>(originalPos.y);
        snapPieceToTileCenter(board, oldCol, oldRow);
    }

    void ::chess::BoardEvent::resetDragState(Board& board) {
        board.pieces[draggedPieceArrayIndex].setScale(originalScale);
        draggedPieceArrayIndex = -1;
    }

    void ::chess::BoardEvent::snapPieceToTileCenter(Board& board, int col, int row) {
    ...
    int ::chess::BoardEvent::getPieceIndexAtTile(Board board, int col, int row) {
        for (size_t i = 0; i < board.pieces.size(); ++i) {
    ...
    int ::chess::BoardEvent::getTargetPieceArrayIndexAtTile(Board& board, int col, int row) {
        for (size_t i = 0; i < board.pieces.size(); ++i) {
            if (i == draggedPieceArrayIndex) {
                continue; // Ignore the dragged piece
            }
